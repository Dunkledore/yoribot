...
class PlantTycoon:
    ...
    async def _send_notification(self, user_id, message):
        ...
        member = await self._get_member(str(user_id))
    ...
    async def _seed(self, context):
        ...
        if author.id not in self.gardeners:
            self.gardeners[str(author.id)] = {}
            self.gardeners[str(author.id)]['current'] = False
            self.gardeners[str(author.id)]['points'] = 0
            self.gardeners[str(author.id)]['badges'] = []
            self.gardeners[str(author.id)]['products'] = {}
        if not self.gardeners[str(author.id)]['current']:
            ...
            if 'water' not in self.gardeners[str(author.id)]['products']:
                self.gardeners[str(author.id)]['products']['water'] = 0
            self.gardeners[str(author.id)]['products']['water'] += 5
            self.gardeners[str(author.id)]['current'] = plant
            await self._save_gardeners()
    ...
    async def _profile(self, context, *, member: discord.Member=None):
        ...
        if author.id in self.gardeners:
            gardener = await self._gardener(str(author.id))
    ...
    async def _state(self, context):
        ...
        author = context.message.author
        gardener = await self._gardener(str(author.id))
        if str(author.id) not in self.gardeners or not gardener.current:
    ...
    async def _buy(self, context, product, amount: int):
        ...
        author = context.message.author
        if str(author.id) not in self.gardeners:
            message = 'You\'re currently not growing a plant.'
        else:
            if product.lower() in self.products and amount > 0:
                cost = self.products[product.lower()]['cost'] * amount
                withdraw_points = await self._withdraw_points(str(author.id), cost)
                if withdraw_points:
                    if product.lower() not in self.gardeners[str(author.id)]['products']:
                        self.gardeners[str(author.id)]['products'][product.lower()] = 0
                    self.gardeners[str(author.id)]['products'][product.lower()] += amount
                    self.gardeners[str(author.id)]['products'][product.lower()] += amount * self.products[product.lower()]['uses']
                    await self._save_gardeners()
    ...
    async def _convert(self, context, amount: int):
        ...
        if self.bank.account_exists(author):
            withdraw_points = await self._withdraw_points(str(author.id), amount)
            if withdraw_points:
    ...
    async def _shovel(self, context):
        ...
        author = context.message.author
        if author.id not in self.gardeners or not self.gardeners[str(author.id)]['current']:
            message = 'You\'re currently not growing a plant.'
        else:
            self.gardeners[str(author.id)]['current'] = False
            message = 'You sucessfuly shovelled your plant out.'
            if self.gardeners[str(author.id)]['points'] < 0:
                self.gardeners[str(author.id)]['points'] = 0
            await self._save_gardeners()
    ...
    async def _water(self, context):
        ...
        if str(author.id) not in self.gardeners or not self.gardeners[str(author.id)]['current']:
            message = 'You\'re currently not growing a plant.'
    ...
    async def _fertilize(self, context, fertilizer):
        ...
        if str(author.id) not in self.gardeners or not self.gardeners[str(author.id)]['current']:
            ...
            await self._add_health(channel, str(author.id), product, product_category)
            ...
            for id in self.gardeners:
                ...
                if gardener.current:
                    ...
                    if health < 0:
                        ...
                        delete = True
                if delete:
                    await discord.User(id=str(id)).send(message)
