...
youtube_dl_options = {'source_address': '0.0.0.0', 'format': 'bestaudio/best', 'extractaudio': True,
                      'audioformat': "mp3", 'outtmpl': '%(id)s', 'noplaylist': True, 'nocheckcertificate': True,
                      'ignoreerrors': True, 'quiet': True, 'no_warnings': True, 'outtmpl': "data/audio/cache/%(id)s",
                      'default_search': 'auto'}
...
class Playlist:
    def __init__(self, server=None, sid=None, name=None, author=None, url=None, playlist=None, path=None,
                 main_class=None, **kwargs):
    ...
    def to_json(self):
        ret = {"author": self.author, "playlist": self.playlist, "link": self.url}
        return ret
    ...
    def can_edit(self, user):
        ...
        return any((is_playlist_author, is_server_owner,))
...
class Downloader(threading.Thread):
    def __init__(self, url, max_duration=None, download=False, cache_path="data/audio/cache", *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.url = url
    ...
    def duration_check(self):
        ...
        if self.max_duration and self.song.duration > self.max_duration:
            ...
            raise MaximumLength(
                "songid {} has duration {} > {}".format(self.song.id, self.song.duration, self.max_duration))
...
class Audio:
    ...
    def __init__(self, bot, player):
        ...
        self.server_specific_setting_keys = ["VOLUME", "VOTE_ENABLED", "VOTE_THRESHOLD", "NOPPL_DISCONNECT"]
    ...
    def _cache_size(self):
        ...
        size = sum(map(lambda s: os.path.getsize(os.path.join(self.cache_path, s)) / 10 ** 6, songs))
        return size
    ...
    async def _create_ffmpeg_player(self, server, filename, local=False):
        ...
        if voice_client is None:
            log.debug("not connected when we should be in sid {}".format(server.id))
            ...
            log.debug("reconnect chan id for sid {} is wrong, fixing".format(server.id))
        ...
        voice_client.audio_player = voice_client.create_ffmpeg_player(song_filename, use_avconv=use_avconv,
                                                                      options=options)
    ...
    async def _download_next(self, server, curr_dl, next_dl):
        ...
        if curr_dl.song.id != next_dl.song.id:
            log.debug("downloader ID's mismatch on sid {}".format(
                server.id) + " gonna start dl-ing the next thing on the queue"
    ...
    async def _guarantee_downloaded(self, server, url):
        ...
        if server.id not in self.downloaders:  # We don't have a downloader
            log.debug("sid {} not in downloaders, making one".format(server.id))
    ...
    async def _join_voice_channel(self, channel):
        ...
        try:
            await asyncio.wait_for(self.bot.join_voice_channel(channel), timeout=5, loop=self.bot.loop)
    ...
    def _make_local_song(self, filename):
        ...
        return Song(name=song, id=filename, title=song, url=filename, webpage_url=filename)
    ...
    def _match_yt_playlist(self, url):
        if not self._match_yt_url(url):
            return False
        yt_playlist = re.compile(r'^(https?\:\/\/)?(www\.)?(youtube\.com|youtu\.?be)'
                                 r'(\/playlist\?).*(list=)(.*)(&|$)')
    ...
    def _playlist_exists(self, server, name):
        return self._playlist_exists_local(server, name) or self._playlist_exists_global(name)
    ...
    def _setup_queue(self, server):
        self.queue[server.id] = {"REPEAT": False, "PLAYLIST": False, "VOICE_CHANNEL_ID": None, "QUEUE": deque(),
    ...
    async def audioset_emptydisconnect(self, ctx):
        ...
        self.set_server_setting(server, "NOPPL_DISCONNECT", not noppl_disconnect)
    ...
    @checks.is_admin()  # cause effect is cross-server
    ...
    async def audiostat_servers(self):
        ...
        await self.bot.say("Currently playing music in {} servers.".format(count))
    ...
    async def cache_minimum(self):
        ...
        await self.bot.say("The cache will be at least {:.3f} MB".format(self._cache_min()))

    @cache.command(name="size")
    async def cache_size(self):
        """Current size of the cache."""
        await self.bot.say("Cache is currently at {:.3f} MB.".format(self._cache_size()))
    ...
    async def playlist_add(self, ctx, name, url):
        ...
        if self._valid_playable_url(url):
            ...
            self._save_playlist(server, name, playlist)
            await self.bot.say("Playlist '{}' saved. Tracks: {}".format(name, len(songlist)))
        else:
    ...
    async def _queue(self, ctx, *, url=None):
        ...
        if self.queue[server.id]["PLAYLIST"]:
            log.debug("queueing to the temp_queue for sid {}".format(server.id))
            self._add_to_temp_queue(server, url)
        else:
            log.debug("queueing to the actual queue for sid {}".format(server.id))
            self._add_to_queue(server, url)
        await self.bot.say("Queued.")
    ...
    async def resume(self, ctx):
        ...
        elif not voice_client.audio_player.is_done() and not voice_client.audio_player.is_playing():
    ...
    def can_instaskip(self, member):
        ...
        return is_server_owner or alone

    @commands.command(pass_context=True, no_pm=True)
    async def sing(self, ctx):
        """Makes Red sing one of her songs"""
        ids = ("zGTkAVsrfg8", "cGMWL8cOeAU", "vFrjMq4aL-g", "WROI5WYBU_A", "41tIUr_ex3g", "f9O2Rjn1azc")
    ...
    async def cache_manager(self):
        while self == self.bot.get_cog("Audio"):
            if self._cache_too_large():
                ...
                log.debug("cache too large ({} > {}), dumping".format(self._cache_size(), self._cache_max()))
                self._dump_cache()
    ...
    async def disconnect_timer(self):
        ...
        while self == self.bot.get_cog('Audio'):
            for vc in self.bot.voice_clients:
                server = vc.server
                if not hasattr(vc, 'audio_player') and (server not in stop_times or stop_times[server] is None):
                    log.debug("putting sid {} in stop loop, no player".format(server.id))
    ...
    async def queue_manager(self, sid):
        ...
        if not self.is_playing(server):
            ...
            if len(temp_queue) > 0:
                next_dl = Downloader(temp_queue.peekleft(), max_length)
            elif len(queue) > 0:
    ...
    async def queue_scheduler(self):
        while self == self.bot.get_cog('Audio'):
            ...
            queue = copy.deepcopy(self.queue)
            for sid in queue:
                if len(queue[sid]["QUEUE"]) == 0 and len(queue[sid]["TEMP_QUEUE"]) == 0:
                    continue
    ...
    async def voice_state_update(self, before, after):
        ...
        if before.mute != after.mute:
            ...
            elif not after.mute and (not vc.audio_player.is_playing() and not vc.audio_player.is_done()):
...
def check_files():
    default = {"VOLUME": 50, "MAX_LENGTH": 3700, "VOTE_ENABLED": True, "MAX_CACHE": 0, "SOUNDCLOUD_CLIENT_ID": None,
               "TITLE_STATUS": True, "AVCONV": False, "VOTE_THRESHOLD": 50, "SERVERS": {}}
...
def setup(bot):
    ...
    if opus is False:
        raise RuntimeError("Your opus library's bitness must match your python installation's"
                           " bitness. They both must be either 32bit or 64bit.")
    elif opus is None:
        raise RuntimeError("You need to install ffmpeg and opus. See \"https://github.com/"
