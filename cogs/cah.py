...
class CardsAgainstHumanity:
    ...
    def memberforname(self, name, guild):
        ...
        newMem = memberForID(memID, guild)
        if newMem:
            return newMem
        return None
        ...
        if len(self.games):
            if id:
                ...
                if game == None:
                    ...
                    # If user, has to be joined from guild chat
                    if not ctx.message.guild:
                        msg = "I couldn't find a game attached to that id.  If you are trying to join a user - run the `{}joincah [user]` command in a channel on a guild you share with that user.".format(ctx.prefix)
                        await self.bot.send_message(ctx.message.channel, msg)
                        return
                    else:
                        # We have a guild - let's try for a user
