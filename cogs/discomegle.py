...
class Discomegle:
    ...
    async def remove_from_pool(self, message):
        ...
        if str(user.id) in self.pool.keys():
            self.pool.pop(str(user.id))
            em = discord.Embed(description="**Leaving discomegle pool.**", colour=self.colour)
            await channel.send(embed = em)
        elif str(user.id) in self.link.keys():
            # put partner back into pool
            partner_id = self.link[str(user.id)]["TARGET_ID"]
            partner_channel = self.link[str(user.id)]["TARGET_CHANNEL"]
            self.pool[str(partner_id)] = partner_channel
            self.link.pop(str(partner_id))
            self.link.pop(str(user.id))
    ...
    async def get_next_user(self, message):
        ...
        if str(user.id) in self.link.keys():
