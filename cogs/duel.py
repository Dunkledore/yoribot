...
class Duel:
    ...
    def get_all_players(self, guild: discord.guild):
        return [self.get_player(m) for m in guild.members]

    def format_display(self, guild, id):
        if id.startswith('r'):
            role = discord.utils.get(guild.roles, id=id[1:])
            if role:
    ...
    def protect_common(self, obj, protect=True):
        ...
        protected = self.duelists.get(guild.id, {}).get("protected", [])
        ...
        if guild.id not in self.duelists:
            self.duelists[guild.id] = {}
        self.duelists[guild.id]['protected'] = protected
        dataIO.save_json(JSON_PATH, self.duelists)
        return True
    ...
    async def _protect_user(self, ctx, user: discord.Member):
        if self.protect_common(user, True):
            await ctx.send("%s has been successfully added to the "
                               "protection list." % user.display_name)
        else:
            await ctx.send("%s is already in the protection list."
                               % user.display_name)
    ...
    async def _unprotect_user(self, ctx, user: discord.Member):
        ...
        if self.protect_common(user, False):
            await ctx.send("%s has been successfully removed from the "
                               "protection list." % user.display_name)
        else:
            await ctx.send("%s is not in the protection list."
                               % user.display_name)
    ...
    async def _protection(self, ctx):
        ...
        duelists = self.duelists.get(guild.id, {})
        member_list = duelists.get("protected", [])
        fmt = partial(self.format_display, guild)
        if member_list:
            ...
            await ctx.send("Currently the list is empty, add more people "
    ...
    async def _duels_list(self, ctx, top: int=10):
        ...
        guild = ctx.message.guild
        server_members = {m.id for m in guild.members}
        if top < 1:
            top = 10
        if guild.id in self.duelists:
            def sort_wins(kv):
                _, v = kv
            ...
            duel_stats = filter(stat_filter, self.duelists[guild.id].items())
            ...
            highscore = ""
            place = 1
            members = {uid: guild.get_member(uid) for uid, _ in topten}  # only look up once each
        ...
        try:
            ...
            for i in range(MAX_ROUNDS):
                ...
                for attacker, defender in order:
                    ...
                    if attacker.member == ctx.message.guild.me:
