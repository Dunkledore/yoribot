...
class EmbedMaker:
    ...
    async def initial_config(self, guild=None):
        """adds default settings for all guilds the bot is in
        when needed and on join"""

        if guild:
            if str(guild.id) not in self.settings:
                self.settings[str(guild.id)] = {'inactive': True,
                                            'usercache': [],
                                            'roles': []
                                            }
                self.save_settings()

            if str(guild.id) not in self.embeds:
                self.embeds[str(guild.id)] = {'embeds': []}
                self.save_embeds()
    ...
    async def embedset(self, ctx):
        ...
        if ctx.invoked_subcommand is None:
            ...
            await ctx.invoke(help_cmd, command='embedset')

    @commands.group(name="savedembed", pass_context=True, no_pm=True)
    async def savedembed(self, ctx):
        """embed tools"""
        if ctx.invoked_subcommand is None:
            help_cmd = self.bot.get_command('help')
            await ctx.invoke(help_cmd, command='savedembed')

    @savedembed.command(name="list", pass_context=True, no_pm=True)
    async def list_embeds(self, ctx):
        """lists the embeds on this guild"""
        ...
        names = []
        for embed in self.embeds[str(guild.id)]["embeds"]:
    ...
    async def embed_toggle(self, ctx):
        ...
        guild = ctx.message.guild
        if str(guild.id) not in self.settings:
            await self.initial_config(guild)
        self.settings[str(guild.id)]['inactive'] = \
            not self.settings[str(guild.id)]['inactive']
        self.save_settings()
        if self.settings[str(guild.id)]['inactive']:
    ...
    @checks.is_admin()
    @savedembed.command(name="remove", pass_context=True, no_pm=True)
    async def remove_embed(self, ctx, name: str):
        """removes an embed"""
        guild = ctx.message.guild
        name = name.lower()
        embeds = self.embeds[str(guild.id)]["embeds"]
        embeds[:] = [e for e in embeds if e.get('name') != name]
        self.embeds[str(guild.id)]["embeds"] = embeds
        self.save_embeds()
        await ctx.send("If an embed of that name existed, it is gone now.")

    @checks.is_owner()
    @savedembed.command(name="removeglobal", pass_context=True)
    ...
    @checks.is_admin()
    @savedembed.command(name="make", pass_context=True, no_pm=True)
    async def make_embed(self, ctx, name: str):
        ...
        name = name.lower()
        for e in self.embeds[str(guild.id)]['embeds']:
            if e.get('name') == name:
                return await ctx.send("An embed by that name exists ")

        if str(author.id) in self.settings[str(guild.id)]['usercache']:
        ...
        await self.contact_for_embed(name, author, guild)

    @checks.is_owner()
    @savedembed.command(name="makeglobal", pass_context=True)
    async def make_g_embed(self, ctx, name: str):
        ...
        if str(author.id) in self.settings["global"]['usercache']:
            return await ctx.send("Finish making your prior embed "
                                      "before making an additional one")
    ...
    @savedembed.command(name="fetch", pass_context=True, no_pm=True)
    ...
    async def contact_for_embed(self, name: str, author, guild=None):
        ...
        if message is None:
            if guild is not None:
                self.settings[str(guild.id)]['usercache'].remove(str(author.id))
            else:
    ...
    async def get_embed(self, name: str, guild_id=None):
        ...
        if guild_id is None:
            guild_id = "global"
        if guild_id in self.embeds:
            for embed in self.embeds[guild_id]["embeds"]:
    ...
    async def save_embed(self, name, title, message, guild=None):
        ...
        if guild is not None:
            self.embeds[str(guild.id)]['embeds'].append(embed)
            self.settings[str(guild.id)]['usercache'].remove(str(author.id))
        else:
