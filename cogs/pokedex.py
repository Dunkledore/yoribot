...
class Pokedex:
    ...
    async def pokedex(self, ctx, *, pokemon: str):
        ...
        try:
            ...
            await ctx.send('A Pokémon with that name could not be found.')
        else:
    ...
    async def _item_pokemon(self, *, item_name: str):
        ...
        try:
            ...
            await ctx.send("An item with that name could not be found.")
        else:
            ...
            with aiohttp.ClientSession() as session:
                async with session.get(tm_url) as response:
                    ...
                    if len(moves) <= 30:
                        ...
                        await ctx.send("```{}```".format(t1))
                        await ctx.send("```{}```".format(t2))
        except IndexError:
            await ctx.send("Oh no! That Pokémon was not available in that generation.")
        except AttributeError:
