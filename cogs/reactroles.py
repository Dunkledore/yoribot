...
Gave a total of {g} roles."""
    ...
    NOT_IN_guild = ":x: The channel must be in a guild."
    ROLE_NOT_FOUND = ":x: Role not found on the given channel's guild."
    EMOJI_NOT_FOUND = ":x: Emoji not found in any of my guilds or in unicode emojis."
    CANT_ADD_REACTIONS = ":x: I don't have the permission to add reactions in that channel."
    CANT_MANAGE_ROLES = ":x: I don't have the permission to manage users' roles in the channel's guild."
    ...
    LINK_NAME_TAKEN = ":x: That link name is already used in the current guild. Remove it before assigning to it."
    UNLINK_NOT_FOUND = ":x: Could not find a link with that name in this guild."
    UNLINK_SUCCESSFUL = ":white_check_mark: The link has been removed from this guild."
    ...
    def __init__(self, bot: discord.Client):
        ...
        self.role_map = {}
        self.role_cache = {}
        self.links = {}  # {guild.id: {channel.id_message.id: [role]}}
    ...
    async def on_message_delete(self, message: discord.Message):
        ...
        if not channel.is_private:
            ...
            guild = channel.guild
            guild_conf = self.get_config(guild.id)
            channel_conf = guild_conf.get(channel.id, {})
            ...
            guild_links = guild_conf.get(self.LINKS_ENTRY)
            if guild_links is not None:
                for links in guild_links.values():
                    if pair in links:
                        links.remove(pair)
    
    async def _init_bot_manipulation(self):
        await self.bot.wait_until_ready()
        for guild_id, guild_conf in self.config.items():
            guild = self.bot.get_guild(guild_id)
            if guild is not None:
                for channel_id, channel_conf in filter(lambda o: o[0] != self.LINKS_ENTRY, guild_conf.items()):
                        ...
                        for msg_id, msg_conf in channel_conf.items():
                            ...
                            if msg is not None:
                                ...
                                for emoji_str, role_id in msg_conf.items():
                                    ...
                                    if role is not None:
                                        self.add_to_cache(guild_id, channel_id, msg_id, emoji_str, role)
                            else:
                                self.logger.warning("Could not find message {} in {}".format(msg_id, channel.mention))
                    else:
                        self.logger.warning("Could not find channel with id {} in guild {}".format(channel_id,
                                                                                                    guild.name))
                link_list = guild_conf.get(self.LINKS_ENTRY)
                if link_list is not None:
                    self.parse_links(guild_id, link_list.values())
            else:
                self.logger.warning("Could not find guild with id {}".format(guild_id))
    ...
    async def _roles_link_list(self, ctx):
        """Lists all reaction links in the current guild"""
        message = ctx.message
        guild = message.guild
        guild_conf = self.get_config(guild.id)
        guild_links = guild_conf.get(self.LINKS_ENTRY, {})
        embed = discord.Embed(title="Role Links", colour=discord.Colour.light_grey())
        for name, pairs in guild_links.items():
            value = ""
        ...
        if len(embed.fields) == 0:
            embed.description = "There are no links in this guild"
    ...
    async def _roles_unlink(self, ctx, name: str):
        ...
        guild_links = guild_conf.get(self.LINKS_ENTRY)
        name = name.lower()
        if guild_links is None or name not in guild_links:
            response = self.UNLINK_NOT_FOUND
        else:
            self.remove_links(guild.id, name)
            del guild_links[name]
            self.save_data()
    ...
    async def _roles_link(self, ctx, name: str, *linked_messages):
        ...
        if len(confimation_msg) > 0:
            ...
            guild_conf = self.get_config(guild.id)
            guild_links = guild_conf.setdefault(self.LINKS_ENTRY, {})
            name = name.lower()
            if name in guild_links:
                response = self.LINK_NAME_TAKEN
            else:
                guild_links[name] = pairs
                self.save_data()
    ...
    async def _roles_add(self, ctx, message_id, channel: discord.TextChannel, emoji, *, role: discord.Role):
        ...
        `emoji` can either be a Unicode emoji or a guild emote
        `role` must be found in the channel's guild"""
        guild = channel.guild
        message = await self.safe_get_message(channel, message_id)
        if message is None:
            response = self.MESSAGE_NOT_FOUND
        else:
            msg_conf = self.get_message_config(guild.id, channel.id, message.id)
            ...
            if emoji_id in msg_conf:
                ...
                response = self.NOT_IN_guild
            else:
                if role.guild != channel.guild:
                    ...
                    emoji = None
                    for emoji_guild in self.bot.guilds:
                        if emoji is None:
                            emoji = discord.utils.get(emoji_guild.emojis, id=emoji_id)
                    try:
    ...
    async def _roles_remove(self, ctx, message_id, channel: discord.TextChannel, *, role: discord.Role):
        ...
        guild = channel.guild
        msg_config = self.get_message_config(guild.id, channel.id, message_id)
    ...
    async def _roles_check(self, ctx, message_id, channel: discord.TextChannel):
        ...
        guild = channel.guild
        msg = await self.safe_get_message(channel, message_id)
        guild_links = self.links.get(guild.id, {})
        if channel.id + "_" + message_id in guild_links:
            await ctx.send( self.CANT_CHECK_LINKED)
        elif msg is None:
            await ctx.send( self.MESSAGE_NOT_FOUND)
        else:
            msg_conf = self.get_message_config(guild.id, channel.id, msg.id)
            if msg_conf is not None:  # Something is very wrong if this is False but whatever
                progress_msg = await ctx.send( "Initializing...")
                given_roles = 0
                checked_count = 0
                ...
                for react in msg.reactions:  # Go through all reactions on the message and add the roles if needed
                    ...
                    role = self.get_from_cache(guild.id, channel.id, msg.id, emoji_str)
                    if role is not None:
                        before = 0
                        after = None
                        user = None
    ...
    async def check_add_role(self, reaction, member):
        ...
        if isinstance(member, discord.Member) and member != self.bot.user:
            # Check whether or not the reaction happened on a guild and prevent the bot from giving itself the role
            guild = channel.guild
            emoji_str = reaction.emoji.id if reaction.custom_emoji else reaction.emoji
            role = self.get_from_cache(guild.id, channel.id, message.id, emoji_str)
            if role is not None:
                await self.add_role_queue(member, role, True,
                                          linked_roles=self.get_link(guild.id, channel.id, message.id))
    
    async def check_remove_role(self, reaction, member):
        message = reaction.message
        channel = message.channel
        if isinstance(member, discord.Member):  # Check whether or not the reaction happened on a guild
            guild = channel.guild
            emoji_str = reaction.emoji.id if reaction.custom_emoji else reaction.emoji
    ...
    async def add_role_queue(self, member, role, add_bool, *, linked_roles=set()):
        key = "_".join((member.guild.id, member.id))  # Doing it this way here to make it simpler a bit
    ...
    def get_link(self, guild_id, channel_id, message_id):
        return self.links.get(guild_id, {}).get(channel_id + "_" + message_id, set())

    def parse_links(self, guild_id, links_list):
        """Parses the links of a guild into self.links
        ...
        for link in links_list:
            ...
            for entry in link:
                ...
                role_list.update(self.get_all_roles_from_message(guild_id, channel_id, message_id))
                link_dict[entry] = role_list
        self.links[guild_id] = link_dict

    def remove_links(self, guild_id, name):
        entry_list = self.get_config(guild_id).get(self.LINKS_ENTRY, {}).get(name, [])
        link_dict = self.links.get(guild_id, {})
        for entry in entry_list:
            if entry in link_dict:
                del link_dict[entry]

    # Cache -- Needed to keep the actual role object in cache instead of looking for it every time in the guild's roles
    def add_to_cache(self, guild_id, channel_id, message_id, emoji_str, role):
        """Adds an entry to the role cache"""
        guild_conf = self.role_cache.setdefault(guild_id, {})
        channel_conf = guild_conf.setdefault(channel_id, {})
        message_conf = channel_conf.setdefault(message_id, {})
        message_conf[emoji_str] = role

    def get_all_roles_from_message(self, guild_id, channel_id, message_id):
        """Fetches all roles from a given message returns an iterable"""
        return self.role_cache.get(guild_id, {}).get(channel_id, {}).get(message_id, {}).values()

    def get_from_cache(self, guild_id, channel_id, message_id, emoji_str):
        """Fetches the role associated with an emoji on the given message"""
        return self.role_cache.get(guild_id, {}).get(channel_id, {}).get(message_id, {}).get(emoji_str)

    def remove_role_from_cache(self, guild_id, channel_id, message_id, emoji_str):
        """Removes an entry from the role cache"""
        guild_conf = self.role_cache.get(guild_id)
        if guild_conf is not None:
            channel_conf = guild_conf.get(channel_id)
    ...
    def remove_message_from_cache(self, guild_id, channel_id, message_id):
    ...
    # Config
    def get_message_config(self, guild_id, channel_id, message_id):
        return self.get_config(guild_id).setdefault(channel_id, {}).setdefault(message_id, {})
    
    def get_config(self, guild_id):
        config = self.config.get(guild_id)
        if config is None:
            self.config[guild_id] = copy.deepcopy(self.guild_DEFAULT)
