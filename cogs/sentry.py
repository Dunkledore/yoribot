...
class Sentry:
    ...
    async def dsn(self, ctx, dsn: str):
        ...
        if re.match('(https\:\/\/|http\:\/\/).*\:.*\@.*\/*[0-9]', dsn) is None:
            await ctx.send('DSN key is not valid. Make sure its a full private key!')
        else:
            self.settings['dsn'] = dsn
            try:
                await self.bot.delete_message(ctx.message)
                await ctx.send('DSN key set, removed your message for safety.\nReload the cog for the changes to have effect.\nTry the key with ``{}sentry test``'.format(ctx.prefix))
            except:
                await ctx.send('DSN key set. Please remove your message for the safety of your logging operation.\nReload the cog for the changes to have effect.\nTry the key with ``{}sentry test``'.format(ctx.prefix))
            self.save_json()
    ...
    async def test(self, ctx, *, message="A test message to Sentry"):
        ...
        try:
            ...
            await ctx.send('Test message should be send. Please check your Sentry instance.')
        except:
    ...
    async def ssl(self, ctx):
        ...
        if self.settings['ssl'] is True:
            ...
            await ctx.send('SSL verification is disabled.\nReload the cog for the changes to have effect.')
        else:
            self.settings['ssl'] = True
            await ctx.send('SSL verification is enabled\nReload the cog for the changes to have effect.')
        self.save_json()
    ...
    async def remove_ignore(self, ctx, logger):
        ...
        if logger in self.settings['ignore']:
            ...
            self.save_json()
            await ctx.send('``{}`` deleted from the ignore list.\nReload the cog for the changes to have effect.'.format(logger))
        else:
            await ctx.send('``{}`` is not in the ignore list.'.format(logger))
