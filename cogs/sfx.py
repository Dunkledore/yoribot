...
class Freesound:
    ...
    async def apikey(self, ctx, key):
        ...
        if self.settings["API_KEY"] != "":
            await ctx.send("{} ` Freesound API key found, overwrite it? y/n`".format(user.mention))
            response = await self.bot.wait_for_message(author=ctx.message.author)
            if response.content.lower().strip() == "y":
                self.settings["API_KEY"] = key
                fileIO(SETTINGS, "save", self.settings)
                await ctx.send("{} ` Freesound API key saved...`".format(user.mention))
            else:
                await ctx.send("{} `Cancled API key opertation...`".format(user.mention))
        else:
            self.settings["API_KEY"] = key
            fileIO(SETTINGS, "save", self.settings)
            await ctx.send("{} ` Freesound API key saved...`".format(user.mention))
    ...
    async def add(self, ctx, url):
        ...
        try:
            ...
            log.debug(e)
            await ctx.send("Something went wrong while getting data from freesound.org, check log")
            return
    ...
    #Soon™
    async def copy_move_tree(self, ctx, source, destination):
        ex = False
        # Copy to new path
        try:
            await ctx.send("`Moving File:  {} to {}`".format(source, destination))
            dir = DIR_TMP+"\\"+n
            #print(dir)
            ...
            if sys.platform.startswith('win'):
                if isinstance(e, WindowsError) and e.winerror == 103:
                    await ctx.send("WinError during copy code: {}".format(str(n)))
                    log.debug('uses Windows special name (%s)' % e)
                if isinstance(e, WindowsError) and e.winerror == 183:
                    await ctx.send("WinError during copy code file/dir already exist: {}".format(str(n)))
        ...
        if cp:
            try:
                await ctx.send("`Delete {} from Import`".format(source))
                shutil.rmtree(source)
            except OSError as e:
                if sys.platform.startswith('win'):
                    log.debug(e)
                if isinstance(e, WindowsError) and e.winerror == 3:
                    await ctx.send(" File/dir Not found: {}".format(str(n)))
                    log.debug('uses Windows special name (%s)' % e)
                # etc....
                ex = True
        if ex:
    ...
    async def edit(self, ctx, file):
        ...
        #print(files[0])
        if files == []:
            await ctx.send("```No result try again.```")
        elif len(files) == 1:
            file_path = files[0]
            await ctx.send("```File found: {}\n\nIs this the correct one? \n y/n ```".format(file_path))
            response = await self.bot.wait_for_message(author=ctx.message.author)
            response = response.content.lower().strip()

            if response.startswith(("no","n")):
                await ctx.send("```Audio edit aborted```")
                return
                ...
                #print(plot)
                if plot != None:
                    await ctx.send("```Erreur fetching plot```")
                    return
                else:
                ...
                try:
                    ...
                    await ctx.send("```Start over and try numbers```")
                    return

                if response >= duration:
                    await ctx.send("```Start over and try a lower value```")
                    return
                elif response < 0.0:
                    await ctx.send("```Start over and try a higher value```")
                    return
                else:
                    start_from = response

                # stop_at
                await ctx.send("``` At what time do you wanna end cutting the file?  format: 00.000 seconds```".format(duration))
                response = await self.bot.wait_for_message(author=ctx.message.author)
                response = response.content.lower().strip()
                try:
                    response = float(response)
                ...
                if response <= start_from:
                    await ctx.send("```Start over and try a higher value beyond start value: {}```".format(str(start_from)))
                    return
                elif response > duration:
                    await ctx.send("```Start over and try a lower value than```".format(duration))
                    return
                else:
                    stop_at = response
                ...
                if not os.path.exists(destination):
                    ...
                    await ctx.send("Chopping audiofile failed: {} | {}".format(source, destination))
                    return
                else:
